...
root.title("___Cricket Shots Detection___")
...
#
lbl = tk.Label(root, text="___Cricket Shots Detection___", font=('times', 35,' bold '), width=65,height=1,bg="Red3",fg="white")
lbl.place(x=0, y=0)
...
def test_model_proc(fn):
    ...
    if fn!="":
        ...
        model = load_model('shot_model.h5')
        ...
        if shot == 0:
            Cd="Pullshot Detected"
            
        elif shot == 1:
            Cd="Sweep Detected"
            
        elif shot == 2:
            Cd="Legglance-flick Detected"
        ...
        return A
    img = open(fn)    
    myfile = open("C:/Code/Code Cricket shot/testing")
    if img == myfile:
          print(myfile)
...
def openimage():
    ...
    global fn
    print(fn)
    fileName = askopenfilename(initialdir='D:/Code/Code Cricket shot/testing', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
